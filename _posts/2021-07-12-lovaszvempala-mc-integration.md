...
4. Proceeding to the main part of the Lovasz Vempala Algorithm, a 2-level nested loop is run. In the outer loop, we keep running the loop until variance $$ a_{i} $$ crosses unity. $$W_{i}$$ for $$i = 1,2,...,m$$ is calculated and multiplied subsequently to $$W_{0}$$ which was earlier calculated as $$volume(K)$$.
